...
void Fractal2D::updateColorField() {
	colorField.clear();
	for(qreal x = minX; x < maxX; x += stepx) {
		for(qreal y = minY; y < maxY; y += stepy) {
			...
			may be we should use
			 colorField.push_back(FractalPoint(x, y, QColor(iter % 64, (iter * iter) % 64,
			 															iter % 256))); // TODO leave only iter, to set brightness
			 ?
			 */
			colorField.push_back(FractalPoint(x * (maxX - minX), y * (maxY - minY),
											  QColor(iter % 64, (iter * iter) % 64,
													 iter % 256))); // TODO leave only iter, to set brightness
		}
	}
}

QVector<FractalPoint> &Fractal2D::getColorFieldR() {
	return colorField;
}
